...
const ReviewClock = () => {
  ...
  const rays = Array.from({ length: 12 }, (_, i) => {
    ...
    const colors = [
      'bg-purple-600', 'bg-purple-500', 'bg-purple-500', 'bg-pink-500', 
    ...
    <div className="relative flex items-center justify-center w-28 h-28 mx-auto mb-8">
      ...
      <div className="absolute w-full h-full">
        {rays.map((ray, index) => (
          <div 
            key={index}
            className={`absolute left-1/2 top-1/2 w-1 h-3 ${ray.color} rounded-full`}
            style={{
      ...
      {/* Clock circle */}
      <div className="absolute w-24 h-24 rounded-full border-2 border-gray-800"></div>
